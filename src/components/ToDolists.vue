    ...
    >
      <div>
        <div class="main">
          ...
          <h4 :class="[list.status ? 'green' : 'red']"  @dblclick="changeStatus(list.status,list._id), (list.status = !list.status)">
            执行状态： {{ list.status }}
          </h4>
          ...
          <span class="soucangjia"
            ><i
              ...
              @click="changeIsFav(list.isFav,list._id), (list.isFav = !list.isFav)"
            ></i
          ></span>
        </div>
        <div class="detail">
          <p class="p-detail">
            {{ list.id }} : {{ list.detail1 }} --> {{ list.detail2 }} -->{{
              list.detail3
            }}
...
import Lists from "../models/MonLists.js";
import Terms from "../models/MonTerms.js";
...
export default defineComponent({
  ...
  setup() {
    ...
    var lists = ref<Lists[]>([]);
    var searchLists = ref<Lists[]>([]);
    var searchValue = ref();
    var timer = ref<any>(0);
    var orderTerm = ref<Terms>("_id");
    const handleTerm = (term: Terms) => {
      orderTerm.value = term;
    };

    onMounted(async (): Promise<void> => {
      let res = await fetch("http://localhost:4500/api/posts/toDoLists");
    ...
    watch(orderTerm, (newValue) => {
      if (newValue === "isFav") {
        searchLists.value.sort((a: Lists, b: Lists) => {
    ...
    const changeStatus = async (bool: boolean,id: number|string): Promise<void> => {      
      // console.log(state);
      try {
        await fetch("http://localhost:4500/api/posts/todolists/" + id, {
          method: "PUT",
          headers: {
    ...
    const changeIsFav = async (bool: boolean,id: number|string): Promise<void> => {
      try {
        await fetch("http://localhost:4500/api/posts/todolists/" +id, {
          method: "PUT",
          headers: {
          ...
          body: JSON.stringify({
            isFav: !bool, //解析 state  作为 fetch api 的body 发送给服务器 更新数据
          }),
        }).then((res) => {
          if (!res.ok) {
